...
bool FUNCTION GETTKN(TokenType REF TOKEN) IS
  ...
  WHILE true DO
    EOL = GETCHR(CHAR);
//    Put(" In GetTkn and CH is ");
//    Put(Char.CH);
//    Put(".  EOL is ");
//    IF EOL THEN PUT("TRUE"); ELSE PUT("FALSE"); END IF;
//      Put(", Char STATE= ");
//      Put(FSATyp'Image(Char.STATE));
//      New_Line;
//      New_Line;
    IF EOL THEN
...
//                       Len = TOKEN.uStr.length() - 1;
                CHAR.ch = TOKEN.uStr.back(); // SO DELIMCH CORRECTLY RETURNS THE ARITH SIGN CHAR
...
//  char ch;
  ...
  IF (TOKEN.STATE EQ ALLELSE) AND (Len > 2) AND (TOKEN.uStr.front() EQ '.') AND isdigit(TOKEN.uStr.at(1)) THEN
    ...
    switch (CHAR.STATE) IS
      ...
      case OP :
                IF ((CHAR.ch NE '+') AND (CHAR.ch NE '-')) OR 
                              ((TOKEN.uStr.length() > 1) AND (TOKEN.uStr.back() NE 'E')) THEN
                  UNGETCHR();
                  goto ExitLoop;
                ENDIF;
                TOKEN.uStr += CHAR.ch;
