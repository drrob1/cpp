...
//          c = wgetch(menu_win);
          c = getch();
          mvprintw(1,1," ch entered is %d.",c);
          switch(c)
          {
            case KEY_MOUSE:
              IF getmouse(ADROF event) EQ OK THEN   /* When the user clicks left mouse button */
                IF event.bstate BITAND BUTTON1_CLICKED THEN  /* When the user clicks left mouse button */
                  mvprintw(1,1," event.x is %d , event.y is %d . ",event.x,event.y);
              ...
              break;
            default :
              mvprintw(30,1, " in default case of KEY_MOUSE.  event id %d, x %d y %d z %d  bstate %x  ",event.id, event.x,event.y,event.z,event.bstate);
          ENDCASE;
        ENDWHILE;               
end:
        endwin();
