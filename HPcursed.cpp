...
13 Feb 15 -- Divided right window into upper and lower, added scrollok(), leaveok(), and used addstr to see how it works.
              And added RegAltered flag to ZeroReg command.
--------------------------------------*/
...
PROCEDURE writehelp() IS
  ...
  getmaxyx(winright,maxrow,maxcol);
  row = maxrow - 31;
  col = 0;
  IF (row < 0) OR (col < 0) THEN
    wprintw(winright," screen is too small.  Cannot output help.\n");
    return;
  ENDIF;
  wattron(winright,COLOR_PAIR(0));
  wmove(winright,row,col);
  wclrtobot(winright);
  wmove(winright,row,col);
  wprintw(winright," This is an RPN style calculator.");
  ++row;
  mvwprintw(winright,row,col," fix, float, gen -- output string format options.\n");
  ++row;
  mvwprintw(winright,row,col," SQRT,SQR -- X = sqrt(X) or sqr(X) register.\n");
  ++row;
  mvwprintw(winright,row,col," STO,RCL  -- store/recall the X register to/from a memory register.\n");
  ++row;
  mvwprintw(winright,row,col,"    Now allows single letter or number to designate the register 0..35\n");
  ++row;
  mvwprintw(winright,row,col,"    Must be first on line, else reg from a different module is used.\n");
  ++row;
  mvwprintw(winright,row,col," ZEROREG -- Zeros all of the registers, but not the stack.\n");
  ++row;
  mvwprintw(winright,row,col," SHOREG -- Shows the non-zero registers in its own part of a window.\n");
  ++row;
  mvwprintw(winright,row,col," SWAP,SWAPXY,<>,><,~,`, side arrow keys -- commands to swap X and Y.\n");
  ++row;
  mvwprintw(winright,row,col," LASTX,@ -- put the value of the LASTX register back into X.\n");
  ++row;
  mvwprintw(winright,row,col," ROLLDN -- roll the stack down one register.  X goes to T1.\n");
  ++row;
  mvwprintw(winright,row,col," Up or Down arrow -- Stack movement commands.\n");
  ++row;
  mvwprintw(winright,row,col," EXP,LN -- evaluate exp(X) or ln(X) and put result back into X.\n");
  ++row;
  mvwprintw(winright,row,col," ^   -- evaluate ABS(Y) to the X power, put result in X.\n");
  ++row;
  mvwprintw(winright,row,col," **  -- like ^ but rounds X before calling the PWRI function.\n");
  ++row;
  mvwprintw(winright,row,col," INT, ROUND, FRAC -- guess.\n");
  ++row;
  mvwprintw(winright,row,col," MOD -- evaluate Y MOD X, put result in X and pop stack 1 reg.\n");
  ++row;
  mvwprintw(winright,row,col," SIN,COS,TAN,ARCTAN,ARCSIN,ARCCOS -- Now in degrees.\n");
  ++row;
  mvwprintw(winright,row,col," D2R, R2D -- perform degrees / radians conversion of the X register.\n");
  ++row;
  mvwprintw(winright,row,col," JUL -- Julian date num Z month, Y day, X year.  Pop stack x2.\n");
  ++row;
  mvwprintw(winright,row,col," TODAY- Julian date num of today's date.  Pop stack x2.\n");
  ++row;
  mvwprintw(winright,row,col," GREG-- Return Z month, Y day, X year of Julian date number in X.\n");
  ++row;
  mvwprintw(winright,row,col," DOW -- Return day number 1..7 of julian date number in X register.\n");
  ++row;
  mvwprintw(winright,row,col," HEX -- Round X register to a longcard and output hex fmt.\n");
  ++row;
  mvwprintw(winright,row,col," PRIME -- Returns TRUE or FALSE of X reg.\n");
  ++row;
  mvwprintw(winright,row,col," HCF -- Pushs highest common factor of Y and X onto stack.\n");
  ++row;
  mvwprintw(winright,row,col," TOCLIP, FROMCLIP -- Send to/from clipboard.\n");
  ++row;
  mvwprintw(winright,row,col," substitutions: = for +, ; for *, ' for +, ',' for -, _ is minus .\n");
  ++row;
  mvwprintw(winright,row,col," tab key -- changes font size 10, 12, 14, 16, 18, 20 then back down\n");
  ++row;
  mvwprintw(winright,row,col," HOL -- holiday days and dates for year in X. \n");
  ++row;
  mvwprintw(winright,row,col," Last Compiled %s.",LastCompiled.c_str());
  wrefresh(winright);
ENDPROC; // writehelp
/*****************************************************************/
...
//  char line[512];  not used anymore.
  ...
  ELSIF inputline.compare("HOL") EQ 0 THEN
    ...
    writestack();
    wrefresh(winleft);
//  break; to end the dummy loop unconditionally.
//  end of dummy loop
    inputline.clear();
    return 0;
