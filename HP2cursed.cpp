...
//  getmaxyx(winrightlower,maxrow,maxcol);
//  row = maxrow - 31;
//  row = 0;
//  col = 0;
...
//    return;
//  ENDIF;
  wattron(winrightlower,COLOR_PAIR(0));
  wmove(winrightlower,row,col);
  wclrtobot(winrightlower);
//  wmove(winrightlower,row,col);
//                                                  waddstr(winrightlower," This is an RPN style calculator. ");
  wprintw(winrightlower,"FIX,FLOAT,GEN -- output format.  SQRT,SQR,STO,RCL,ZEROREG,SHOREG\n");
  wprintw(winrightlower,"SWAP,SWAPXY,<>,><,~,` -- equivalent.  LASTX,@ -- Equivalent commands.\n");
  wprintw(winrightlower,"ROLLDN -- X goes to T1.  Arrow keys -- Stack movement commands.\n");
  wprintw(winrightlower,"EXP,LN,INT,ROUND,FRAC,MOD,HEX,PRIME,D2R,R2D,trig functions in deg.\n");
  wprintw(winrightlower,"^ -- evaluate Y^X, put result in X.  ** -- abs Y, rounds X.\n");
  wprintw(winrightlower,"JUL,GREG -- Z month, Y day, X year.  Pop stack as needed.\n");
  wprintw(winrightlower,"TODAY -- Julian date of today's date.\n");
  wprintw(winrightlower,"DOW -- Julian date number in X register.\n");
  wprintw(winrightlower,"HCF -- Pushs highest common factor of Y and X onto stack.\n");
  wprintw(winrightlower,"substitutions: = for +, ; for *, ' for +, ',' for -, _ is minus .\n");
  wprintw(winrightlower,"HOL -- holiday days and dates for year in X.");
  wrefresh(winrightlower);
ENDPROC; // writehelp
/*****************************************************************/
