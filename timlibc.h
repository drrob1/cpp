...
/*    long int ElapsedSec; not used by libc.  Probably what the Ada lib calls rawtime.   */
...
string FUNCTION MDY2STR(int M, int D, int Y, string REF MDYSTR);

PROCEDURE TIME2MDY(int REF M,int REF D,int REF Y);
...
RETURN unsigned int FUNCTION JULIAN(int M, int D, int Y);

PROCEDURE GREGORIAN(unsigned int Juldate,int REF M,int REF D,int REF Y);
